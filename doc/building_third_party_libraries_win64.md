...
-o tmp/output/duktape.o -c tmp/output/duktape.c
	x86_64-w64-mingw32-gcc -o tmp/output/duktape.dll -s 
-shared tmp/output/duktape.o -Wl,--out-implib,tmp/output/libduktape.dll.a
	ar rcs tmp/output/libduktape.a tmp/output/duktape.o
	x86_64-w64-mingw32-gcc -Wall -mmmx -msse -msse2 -mfpmath=sse -O2 
-o tmp/output/duk_module_node.o -c extras/module-node/duk_module_node.c 
-Itmp/output
	x86_64-w64-mingw32-gcc -o tmp/output/duk_module_node.dll -s 
-shared tmp/output/duk_module_node.o 
...
LIBS="-lbz2 -lpng"
	make LIBS="-L/usr/local/x86_64-w64-mingw32/lib 
/usr/local/x86_64-w64-mingw32/lib/libfreetype.a -lz -lmingw32 -lSDL2main
/usr/local/x86_64-w64-mingw32/lib/libSDL2.dll.a -ldinput8 -ldxguid -ldxerr8 
-luser32 -lgdi32 -lwinmm -limm32 -lole32 -loleaut32 -lshell32 -lversion -luuid 
-lpng -lbz2"
	make install
```
...
```
	x86_64-w64-mingw32-gcc -c utf8.c -o utf8.o -O2 -mmmx -msse -msse2 -mfpmath=sse
-Wall
...
```
	./configure --prefix=/usr/local/x86_64-w64-mingw32 --disable-xz 
--disable-xzdec
	make
	make install
```
