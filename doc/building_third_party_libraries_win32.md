...
```
	...
	gcc -Wall -mmmx -mfpmath=387 -O2 -o tmp/output/duktape.o 
...
```
	./configure --prefix=/usr/local/mingw32 --enable-static --disable-xmms-plugin
--disable-oggtest --with-ogg-includes=/usr/local/mingw32/include 
...
```
	./configure --prefix=/usr/local/mingw32 CFLAGS="-W -Wall -O2 
-I/usr/local/mingw32/include" CPPFLAGS="-I/usr/local/mingw32/include" 
...
CPPFLAGS="-I/usr/local/mingw32/include" LDFLAGS="-s -L/usr/local/mingw32/lib" 
...
```
	...
	mingw32-gcc -o tigr.dll tigr.o -shared -s -Wl,--out-implib,libtigr.dll.a 
-ld3d9
